/**
 * @file gemm_cuda_v1.0.0.cu
 * @brief CUDA implementation of GEMM (v1.0.0 - Basic)
 * @details Basic CUDA kernel with simple tiling
 */
...
/**
 * @brief Basic GEMM kernel without shared memory
 */
__global__ void gemm_kernel_basic(int M, int N, int K,
...
/**
 * @brief GEMM kernel with shared memory tiling
 */
__global__ void gemm_kernel_tiled(int M, int N, int K,
                                  double alpha, const double* A, int lda,
                                  const double* B, int ldb,
                                  double beta, double* C, int ldc) {
...
int main(int argc, char* argv[]) {
    ...
    int use_tiled = (argc > 5) ? atoi(argv[5]) : 1;  // 0=basic, 1=tiled
    
    printf("========================================\n");
    printf("CUDA GEMM Implementation v1.0.0\n");
    printf("========================================\n");
    ...
    printf("Kernel: %s\n", use_tiled ? "Tiled with shared memory" : "Basic");
    ...
    if (use_tiled) {
        gemm_kernel_tiled<<<gridDim, blockDim>>>(M, N, K, alpha, d_A, K, d_B, N, beta, d_C, N);
    } else {
        gemm_kernel_basic<<<gridDim, blockDim>>>(M, N, K, alpha, d_A, K, d_B, N, beta, d_C, N);
    }
